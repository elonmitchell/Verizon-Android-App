        ...
        goLogin: function(e){    
        	...
        	$.post( App.config.server + App.config.loginAPI, { username: username, password: password }).done(function( data ) {
				...
				switch(statusCode){
					...
					case 4:
						console.log('LoginView::login::Login failed (Status Code=' + statusCode + ').');
						self.$('#alert-box').html('<p>This account is locked. Please contact the administrator.</p>').show();
						l.stop();
						break;
						
					case 5:
						console.log('LoginView::login::Login failed (Status Code=' + statusCode + ').');
						self.$('#alert-box').html('<p>Could not validate user. Please contact the administrator.</p>').show();
						l.stop();
						break;
						
					case 6:
						console.log('LoginView::login::Login failed (Status Code=' + statusCode + ').');
						self.$('#alert-box').html('<p>Unknow exception. Please contact the administrator.</p>').show();
						l.stop();
						break;
